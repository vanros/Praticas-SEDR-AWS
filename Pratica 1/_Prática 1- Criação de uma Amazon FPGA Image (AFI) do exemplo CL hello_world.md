...
```bash 
$ aws ec2 create-key-pair --key-name **_nomeDaKeyPair-key_** --query 'KeyMaterial' --output text > nomeDaKeyPair-key.pem
```
...
1. Criar e conectar uma instância t2.2xlarge com o Ambiente de desenvolvimento **FPGA Developer AMI.**
 ...
   ```bash
   $ aws ec2 run-instances --image-id ami-626e9918 --security-group-ids **_sg-6fc17419_** --count 1 --instance-type t2.2xlarge --key-name nomeDaKeyPair --query 'Instances[0].InstanceId'
   ```
